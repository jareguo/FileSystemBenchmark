//var exec = require("child_process").exec;
var fs = require("fs");
var url = require("url");
var path = require('path');

if (process.platform == 'win32') {
    var win32 = require('../win32');
}
var mime = require('./mime');

function navigate(response, request) {
    var dir = url.parse(request.url, true).query.path;
    if (dir) {
        //console.log('navigate to : ' + dir);
        fs.readdir(dir, function (error, items) {
            if (error) {
                response.writeHead(500, { "Content-Type": "text/plain" });
                response.end(error + "\n");
                return;
            }
            global.statFiles(dir, items, function () {
                var json = JSON.stringify({
                    items: items,
                });
                response.writeHead(200, { "Content-Type": "application/json" });
                response.end(json);
            });
        });
    }
}

function navigateSync(response, request) {
    var dir = url.parse(request.url, true).query.path;
    if (dir) {
        //console.log('navigate to : ' + dir);
        var items = fs.readdirSync(dir);
        for (var i = 0; i < items.length; ++i) {
            var subPath = path.join(dir, items[i]);
            items[i] = { name : items[i] };
            //console.log(subPath);
            try {
                var stat = fs.statSync(subPath);
            }
            catch (ex) {
                items[i].type = "undefined";
                continue;
            }
            if (stat.isDirectory()) {
                items[i].type = "folder";
            }
            else {
                items[i].type = "file";
            }
        }

        var json = JSON.stringify({
            items : items,
        });
        response.writeHead(200, { "Content-Type": "application/json" });
        response.end(json);
    }
}

function getDrives(response, request) {
    var self = this;
    if (win32) {
        console.log('get drives');
        win32.getDrives(function (drives) {
            for (var i = 0; i < drives.length; ++i) {
                drives[i] = mime.stat(drives[i]);
            }
            var json = JSON.stringify({
                files: drives,
            });
            response.writeHead(200, { "Content-Type": "application/json" });
            response.end(json);
        });
    }
    else {
        response.writeHead(500, { "Content-Type": "text/plain" });
        response.end("not support windows\n");
    }
}

function open(response, request) {
    var query = url.parse(request.url, true).query;
    var filepath = query.path;
    var length = parseInt(query.length);
    //console.log("open " + filepath);
    fs.open(filepath, "rs", 0666, function (err, fd) {
        if (err) {
            response.writeHead(500, { "Content-Type": "text/plain" });
            response.end("failed to open " + filepath + '\n' + err);
            return;
        }
        var buffer = new Buffer(length);
        buffer.fill(0);
        fs.read(fd, buffer, 0, length, null, function (err, length, buffer) {
            console.assert(!err);
            fs.close(fd);

            response.writeHead(200, { 'Content-Length' : length });
            response.end(buffer);
        });
    });
}

function openSync(response, request) {
    var query = url.parse(request.url, true).query;
    var filepath = query.path;
    var length = parseInt(query.length);
    //console.log("open " + filepath);
    try {
        var fd = fs.openSync(filepath, "rs");
    }
    catch (ex) {
        fs.closeSync(fd);
        response.writeHead(500, { "Content-Type": "text/plain" });
        response.end("failed to open " + filepath + '\n' + err);
        return;
    }
    //try {
        global.buffer.fill(0);
        var length = fs.readSync(fd, global.buffer, 0, length);
        //console.log(global.buffer);
    //}
    //finally {
        fs.closeSync(fd);
    //}
    response.writeHead(200, { 'Content-Length' : length });
    response.end(global.buffer);
}

exports.navigate = navigate;
exports.navigateSync = navigateSync;
exports.getDrives = getDrives;
exports.open = open;
exports.openSync = openSync;